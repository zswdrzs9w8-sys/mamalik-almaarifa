...
const clean=s=>String(s||'').toLowerCase().normalize('NFD').replace(/[\u064B-\u065F\u0670]/g,'').replace(/[أإآ]/g,'ا').replace(/ة/g,'ه').replace(/ى/g,'ي').replace(/[^\p{L}\p{N}]/gu,'');
const correct=(given,answer)=>{let a=clean(given),b=clean(answer);return a.length>1&&(a===b||b.includes(a)||a.includes(b))};
...
async function api(req,res,url){
 ...
 if(a==='submit'&&g.question){let ok=correct(b.text,g.question.answer);g.lastResult={correct:ok,given:b.text,answer:g.question.answer};if(ok){for(const i of g.targets)g.lands[i].owner=g.turn;let n=g.lands.filter(l=>l.owner===g.turn).length;if(n>=13){g.phase='finished';g.winner=g.turn}else next(g)}else{g.phase='steal';g.note='إجابة غير صحيحة — انتقل السؤال للدولة الأخرى، والمشاورة مسموحة';g.endsAt=Date.now()+30000;}return json(res,200,publicState(g,true));}
 if(a==='steal'&&g.phase==='steal'){let ok=correct(b.text,g.question.answer);g.lastResult={correct:ok,given:b.text,answer:g.question.answer};if(ok)g.lands[g.targets[0]].owner=1-g.turn;next(g);return json(res,200,publicState(g,true));}
