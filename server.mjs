...
function next(g){g.turn=1-g.turn;let c=g.countries[g.turn];if(!c.bag.length)c.bag=shuffle(c.players.map((_,i)=>i));g.player=c.bag.pop();g.phase='draw';g.question=null;g.targets=[];g.double=false;g.options=null;g.note='جاري سحب اللاعب...';}
...
async function api(req,res,url){
 ...
 if(a==='target'){if(g.phase!=='choose')return json(res,400,{error:'ليس وقت الاختيار'});let i=+b.index,own=g.lands.map((l,j)=>l.owner===g.turn?j:-1).filter(j=>j>=0);if(!own.some(j=>adj(j,i)))return json(res,400,{error:'اختر أرضًا ملاصقة لدولتك'});g.targets=[i];g.phase='briefing';g.note='قرر إن كنت ستستخدم دبلها، ثم ابدأ السؤال';return json(res,200,publicState(g,true));}
 if(a==='start'&&g.phase==='briefing'){g.phase='difficulty';g.note='اختر مستوى السؤال';return json(res,200,publicState(g,true));}
 if(a==='helper'){let h=b.helper,c=g.countries[g.turn];if(!c.helps[h])return json(res,400,{error:'المساعدة منتهية'});if(h==='double'){if(g.phase!=='briefing'||!g.targets.length)return json(res,400,{error:'اختر الأرض الأولى قبل دبلها'});let i=+b.index;if(!adj(g.targets[0],i)||i===g.targets[0])return json(res,400,{error:'الأرض الثانية يجب أن تلاصق الأولى'});g.targets.push(i);g.double=true;}else if(!g.question)return json(res,400,{error:'ابدأ السؤال أولًا'});c.helps[h]--;if(h==='choices')g.options=choicesFor(g.question,QUESTIONS);if(h==='consult')g.note='سُمح بالمشاورة لهذا السؤال';return json(res,200,publicState(g,true));}
 ...
 if(a==='submit'&&g.question){let ok=correct(b.text,g.question.answer);g.lastResult={id:Date.now(),correct:ok,final:ok,given:b.text,answer:g.question.answer};if(ok){for(const i of g.targets)g.lands[i].owner=g.turn;let n=g.lands.filter(l=>l.owner===g.turn).length;if(n>=13){g.phase='finished';g.winner=g.turn}else next(g)}else{g.phase='steal';g.note='انتقل السؤال للدولة الأخرى، والمشاورة مسموحة';g.endsAt=Date.now()+30000;}return json(res,200,publicState(g,true));}
